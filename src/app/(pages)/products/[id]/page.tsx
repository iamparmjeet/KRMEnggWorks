...
const relatedProducts = [
  { id: 1, name: 'Bar Cutting Machine Model-GQ 52', price: '₹64,000.00', image: '🏗️' },
]
  ...
  return (
    <div className="w-full bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 text-sm text-slate-600">
        ...
        <Link href="/products" className="hover:text-slate-900">Bar Cutting Machine</Link>
        <span className="mx-2">/</span>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            <div className="bg-white border border-slate-200 rounded-lg p-8 h-96 flex items-center justify-center mb-4 relative">
              ...
              <button className="absolute top-4 right-4 bg-white border border-slate-200 p-2 rounded hover:bg-slate-50">
                🔍
              </button>
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-blue-600 font-semibold text-lg mb-4">₹64,000.00</p>
            </div>

            <div className="pb-4 border-b border-slate-200">
              <p className="text-slate-700 text-sm font-semibold">Category: <span className="text-slate-600">{product.category}</span></p>
            </div>
        ...
        <div className="mt-16 bg-white">
          <h2 className="text-2xl font-bold text-blue-600 mb-6">Additional Information</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(product.specifications).map(([key, value]) => (
              <div key={key} className="pb-3 border-b border-slate-200">
                <p className="text-slate-700 text-sm">
                  <span className="font-semibold">• {key}:</span> <span className="text-slate-600">{value}</span>
                </p>
              </div>
            ))}
          </div>
              ...
              <Link key={relProduct.id} href={`/products/${relProduct.id}`}>
                <div className="border border-slate-200 rounded-lg p-4 hover:shadow-lg transition-shadow">
                  <div className="bg-white h-48 flex items-center justify-center rounded border border-slate-200 mb-4">
                    <span className="text-6xl">{relProduct.image}</span>
                  </div>
                  <h3 className="font-semibold text-slate-900 mb-2 text-sm">{relProduct.name}</h3>
                  <p className="text-yellow-600 font-bold mb-3">{relProduct.price}</p>
